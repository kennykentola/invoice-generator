...
module.exports = async (req, res) => {
  ...
  try {
    ...
    doc.on('end', async () => {
      ...
      // Generate WhatsApp link (no download URL in serverless)
      ...
      res.status(200).json({
        ...
        whatsappLink,
        pdfDataUri,
        managerSign: '', // Manager signature handled in PDF, no separate URL
      });
    });
    ...
    try {
      logoBuffer = fs.readFileSync(path.join(__dirname, 'images', 'logo.png'));
    ...
    try {
      electronicsBuffer = fs.readFileSync(path.join(__dirname, 'images', 'gen.jpg'));
      doc.image(electronicsBuffer, 20, 20, { width: 100 }); // Left side
    } catch (err) {
      console.warn('Electronics image not found:', err.message);
    }
    try {
      generatorFanBuffer = fs.readFileSync(path.join(__dirname, 'images', 'fan.jpg'));
      generatorHomeBuffer = fs.readFileSync(path.join(__dirname, 'images', 'home.png'));
    ...
    if (customerSign && customerSign.startsWith('data:image/')) {
      try {
        ...
        doc.image(customerSignBuffer, 50, signY + 10, { width: 150 });
      } catch (err) {
        console.error('Error adding customer signature:', err.message);
        doc.text('_____________________', 50, signY + 10);
      }
    } else {
      doc.text('_____________________', 50, signY + 10);
    }
    ...
    // Manager Signature
    try {
      managerSignBuffer = fs.readFileSync(path.join(__dirname, 'images', 'manager_signature.png'));
      doc.image(managerSignBuffer, 350, signY + 10, { width: 150 });
    } catch (err) {
      console.warn('Manager signature not found, using fallback text:', err.message);
      doc.fontSize(12).fillColor('blue').text("D'more Tech", 350, signY + 10, { align: 'right' });
    }
