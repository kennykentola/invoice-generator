  ...
  try {
    ...
    try {
      ...
      const mailOptions = {
        ...
        html: `
          <p>Dear ${buyer},</p>
          <p>Your invoice is ready. Total: ₦${total.toLocaleString()}</p>
      ...
      res.json({
        ...
        whatsappLink: `https://wa.me/${phone}?text=${encodeURIComponent(
          `Hello ${buyer}, your invoice from D'MORE TECH. Amount is ₦${total}.`
        )}`,
        pdfDataUri,
        downloadURL: url,
      });
    ...
    console.error("❌ Error in /api/invoice:", {
      message: err.message,
      stack: err.stack,
...
// HTML generator (simplified)
function generateInvoiceHTML({ buyer, items, total, totalInWords, customerSign, managerSign }) {
  const date = new Date();
  return `
    <!DOCTYPE html>
    <html>
