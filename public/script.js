...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const totalWords = convertNumberToWords(total);
    const customerSignFile = customerSignInput.files[0];
    const customerSign = customerSignFile ? await fileToBase64(customerSignFile).catch(err => {
      console.warn(`Failed to convert customer signature to Base64: ${err.message}`);
      return "";
    }) : "";
    ...
    try {
      ...
      const data = await response.json();
      const { whatsappLink, downloadURL, pdfBase64 } = data;
      ...
      const gmailBtn = `<a href="mailto:${email}?subject=Invoice from D'MORE TECH&body=Download your invoice: ${downloadURL}" class="btn">📧 Gmail</a>`;
      const whatsappBtn = `<a href="${whatsappLink}" target="_blank" class="btn">📲 WhatsApp</a>`;
      const saveBtn = `<button onclick="saveToLocal()" class="btn">💾 Save</button>`;
      const previewBtn = `<button onclick="previewPDF('${pdfBase64}')" class="btn">👁️ Preview PDF</button>`;
      const downloadBtn = `<a href="${downloadURL}" download="invoice.pdf" class="btn">⬇️ Download Invoice</a>`;
  ...
  function generateInvoiceHTML(payload) {
    const { buyer, items, total, customerSign } = payload;
    const date = new Date();
    ...
    return `
      <div id="final-invoice">
        ...
        <div class="invoice-total">
          <p style="text-align: right;"><strong>TOTAL ₦</strong> ${total.toLocaleString()}</p>
          <p><strong>Amount in words:</strong> ${convertNumberToWords(total)}</p>
        </div>
        <p class="invoice-note">Please Note: Goods sold in good condition are not returnable or exchanged. Thanks for your patronage.</p>
        <div class="invoice-signatures">
          <div class="signature-block">
            <p class="signature-label">Customer Signature</p>
            ${customerSign ? `<img src="${customerSign}" alt="Customer Signature" class="signature-img" />` : '<p class="pending">No signature provided</p>'}
          </div>
          <div class="signature-block">
            <p class="signature-label">Manager Signature</p>
            <img src="/images/manager_signature.png" alt="Manager Signature" class="signature-img" />
          </div>
        </div>
      </div>
    `;
